...
def save_BScope_converted_dataframes(final_table, output_file_path, object_name, output_type, context, compression, line_terminator, header=True,  report = True):
		...
		final_table.sort_index(axis = 0, level = [0, 1], sort_remaining = True) ## level = [0, 1] combined with sort_remaining = True allows to sort according multi-index level 0(here chr) and then level 1 (here position)

		#use the make_path function to create a file name and a path where to save the output
		path_ = make_path(output_file_path = output_file_path, sample_name = object_name, file_extension = "tsv", compression = "gzip", supplemental_info = "{}_{}_format_BScope.BS_conversion_output".format(context, output_type))
				
		print("The output is saved at", path_)

		#save final_table
		final_table.to_csv(path_or_buf = path_, sep = "\t", header = header, compression = compression, line_terminator  = line_terminator,  encoding = "utf-8")
...
def BS_report(input_, input_type, output_file_path, output_file_name, context):
		...
		methylation_table = pd.concat(pandas_dataframe_methylation_list, axis = 1).dropna(axis = 0, how = "any")
	...
	def BS_conversion(self, output_type, output_file_path, min_coverage, report = True, header = False, compression = "gzip", line_terminator = "\n"):
		...
		if self.input_type == "bssnper":
			...
			for object_, object_name in self.input: #iterate over the different pandas dataframe objects provided as input (self.input attribute) and apply the transformation
				...
				save_BScope_converted_dataframes(final_table = final_table , output_file_path = output_file_path, object_name = object_name, output_type = output_type, context = self.context, compression = compression, line_terminator  = line_terminator, report = report)
			...
			if output_type == "methylkit":
				...
				for object_, object_name in self.input:
					...
					new_DataFrame["chr"] = object_.loc[:, "#CHROM"]
					...
					# filter for  minimun read coverage
					cov_filter = new_DataFrame[:, coveragecol_name] >= min_coverage
					...
					final_table = new_DataFrame.dropna(axis=0, how="any", inplace = True)
					...
					if report:
						...
						return output_pandas_dataframes_list.append(final_table)
